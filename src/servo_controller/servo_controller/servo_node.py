...
class ServoController(Node):
    def __init__(self):
        ...
        self.set_servo_angle(self.servo_angle)  # Move servo to center on init

        self.is_found = False
        self.obj_pos = 0.0
        self.locked_on = False  # Track locked_on state
        ...
        self.create_subscription(Bool, '/locked_on', self.locked_on_callback, 10)  # NEW
    ...
    def set_servo_angle(self, angle):
        pulse = self.angle_to_pwm(angle)
        self.set_pwm(self.servo_channel, 0, pulse)
        self.get_logger().info(f"Set servo angle to {angle:.1f}°")
    ...
    def locked_on_callback(self, msg):  # NEW
        self.locked_on = msg.data
        if not self.locked_on:
            # If not locked on, immediately move servo to center
            self.servo_angle = 90.0
            self.set_servo_angle(self.servo_angle)

    def control_loop(self):
        if not self.locked_on:
            # Not locked: hold at center position, no scanning or tracking
            if self.servo_angle != 90.0:
                self.servo_angle = 90.0
                self.set_servo_angle(self.servo_angle)
            # publish normalized angle anyway
            norm_angle = 0.5  # center normalized between 0 and 1
        else:
            # Locked on: proceed with scanning or tracking
            if not self.is_found:
                # Scan mode: sweep servo left and right
                ...
                # Track mode: move servo based on obj_pos
            ...
            # Calculate normalized angle for publishing
