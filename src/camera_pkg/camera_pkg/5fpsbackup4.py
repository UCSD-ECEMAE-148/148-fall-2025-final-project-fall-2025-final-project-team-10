...
class ReidNode(Node):
    ...
    def run_loop(self):
        ...
        if best_rect is not None:
            ...
            # If locked and detected correct label, publish /is_found = True and obj_pos
            if not self.capturing and best_label == self.locked_label:
                is_found_msg = Bool()
                is_found_msg.data = True
                self.is_found_pub.publish(is_found_msg)

                # Normalize x position from center of bbox to range -20 (left) to 20 (right)
                ...
                # Publish False if no matching detection
                is_found_msg = Bool()
                is_found_msg.data = False
                self.is_found_pub.publish(is_found_msg)

        else:
            # No detections at all, publish False
            is_found_msg = Bool()
            is_found_msg.data = False
            self.is_found_pub.publish(is_found_msg)
