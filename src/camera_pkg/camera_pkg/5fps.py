...
class ReidNode(Node):
    def __init__(self):
        ...
        self.locked_on_pub = self.create_publisher(Bool, '/locked_on', 10)  # NEW publisher for locked_on
        self.stop_cmd_pub = self.create_publisher(Twist, '/stop/cmd_vel', 10)  # NEW publisher for stop command

        self.sub_lock = self.create_subscription(Empty, '/reid/lock', self.lock_callback, 10)
        self.bridge = CvBridge()

        # Timing variables for delayed locked_on
        self.lock_time = None
        self.lock_delay = 5.0  # seconds delay after locking
    ...
    def run_loop(self):
        ...
        if self.trigger_lock and best_rect is not None:
            ...
            self.trigger_lock = False
            self.lock_time = time.time()  # record lock time here

        # Publish /locked_on with 5 second delay after locking
        locked_on_msg = Bool()
        current_time = time.time()
        if not self.capturing:
            if self.lock_time and (current_time - self.lock_time) >= self.lock_delay:
                locked_on_msg.data = True
            else:
                locked_on_msg.data = False
        else:
            locked_on_msg.data = False

        self.locked_on_pub.publish(locked_on_msg)

        # Publish stop command if not locked or within 5-second delay window
        if self.capturing or (self.lock_time and (current_time - self.lock_time) < self.lock_delay):
            stop_msg = Twist()
            ...
            self.stop_cmd_pub.publish(stop_msg)
        # else no stop command published (locked and delay passed)
